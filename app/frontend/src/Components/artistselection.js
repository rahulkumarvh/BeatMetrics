...
function ArtistSelection() {
  ...
  useEffect(() => {
    fetch(
      ...
      .then((data) => {
        setArtists(data.artists || []); // Update state with genres data
      })
      .catch((error) => {
  ...
  const toggleGenreSelection = (artist_id) => {
  ...
  return (
    <div className="genres">
      <div className="choose">
        ...
        <h3>Pick 3 of your favourite Artitst !!</h3>
        <div className="buttons">
          <button onClick={() => navigate("/genreselection")}>PREVIOUS </button>
      ...
      <div className="genre-container">
        {rows.map((row, rowIndex) => (
          <div key={rowIndex} className="genre-row">
            {row.map((index) => {
              const artistObject = artists[index];
              const isSelected = selectedArtists.includes(
              ...
              return (
                <div
                  ...
                  onClick={() => toggleGenreSelection(artistObject?.artist_id)}
                >
                  {artistObject ? artistObject.artist_name : "error"}
                </div>
              );
            })}
          </div>
