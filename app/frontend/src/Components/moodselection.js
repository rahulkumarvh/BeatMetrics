...
function MoodSelection() {
  ...
  useEffect(() => {
    fetch(
      ...
      .then((data) => {
        setMoods(data.mood || []); // Update state with genres data
      })
      .catch((error) => {
  ...
  const toggleGenreSelection = (mood_id) => {
    if (selectedMoods.includes(mood_id)) {
      setSelectedMoods(selectedMoods.filter((id) => id !== mood_id));
    } else if (selectedMoods.length < 3) {
      setSelectedMoods([...selectedMoods, mood_id]);
    }
  };
    ...
    try {
      const response = await fetch(
        "https://backend-dot-cs411-bytemysql.uc.r.appspot.com/set_preference",
        {
          ...
          body: JSON.stringify({ mood: selectedMoods }),
        }
      );
  ...
  return (
    <div className="genres">
      <div className="choose">
        ...
        <h3>Pick 3 of your favourite Moods !!</h3>

        <div className="buttons">
          <button onClick={() => navigate("/artistselection")}>
            PREVIOUS{" "}
          </button>
      ...
      <div className="genre-container">
        {rows.map((row, rowIndex) => (
          <div key={rowIndex} className="genre-row">
            {row.map((index) => {
              const moodObject = moods[index];
              const isSelected = selectedMoods.includes(moodObject?.mood_id);
              return (
                <div
                  key={index}
                  className={`genre-circle ${isSelected ? "selected" : ""}`}
                  style={{ backgroundColor: getRandomColor() }}
                  onClick={() => toggleGenreSelection(moodObject?.mood_id)}
                >
                  {moodObject ? moodObject.mood_name : "error"}
                </div>
              );
            })}
          </div>
